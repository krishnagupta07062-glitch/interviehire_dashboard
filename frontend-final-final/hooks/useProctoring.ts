...
import type { ProctoringPayload, Severity } from './shared-types';
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      const tick = () => {
        ...
        if (!currentVideo || !faceTask || !objectTask || currentVideo.readyState < 2 || currentVideo.currentTime <= 0) {
        ...
        try {
          ...
          console.warn('Face detect error', error);
          setState((current) => ({ ...current, status: 'Face detector unavailable' }));
        }
        ...
        try {
          ...
          console.warn('Object detect error', error);
          setState((current) => ({ ...current, status: 'Object detector unavailable' }));
        }
